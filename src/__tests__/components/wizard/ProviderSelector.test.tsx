...
const mockGetPreferredOllamaModel = vi.fn((models: string[]) =>
  models.find((model) => model.startsWith("phi4-mini")) || models[0] || null
);

vi.mock("@/services/tauri-bridge", () => ({
  checkOllamaStatus: () => mockCheckOllamaStatus(),
  getPreferredOllamaModel: (models: string[]) => mockGetPreferredOllamaModel(models),
}));
...
describe("ProviderSelector", () => {
  ...
  beforeEach(() => {
    ...
    mockCheckOllamaStatus.mockResolvedValue({
      ...
      running: true,
      version: "0.1.0",
      models: ["llama3.2:latest", "phi4-mini:latest", "mistral"],
    });
  });
  ...
  describe("Local AI status", () => {
    it("shows Ready status when Local AI is available with models", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Ready (3 models available)")).toBeInTheDocument();
      });
    });
  ...
  describe("Local AI model selection", () => {
    ...
    it("auto-selects first model when switching to Local AI", async () => {
      ...
      expect(onOllamaModelChange).toHaveBeenCalledWith("phi4-mini:latest");
    });
