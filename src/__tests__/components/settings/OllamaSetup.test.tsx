...
vi.mock("@/services/tauri-bridge", () => ({
  ...
  DEFAULT_RECOMMENDED_OLLAMA_MODEL: "phi4-mini",
...
describe("OllamaSetup", () => {
  ...
  describe("no models state", () => {
    ...
    it("should display recommended models", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole("button", { name: /phi4-mini/i })).toBeInTheDocument();
    ...
    it("should call pullOllamaModel when Download Recommended Model button is clicked", async () => {
      ...
      await user.click(screen.getByRole("button", { name: /phi4-mini/i }));

      await waitFor(() => {
        expect(mockPullOllamaModel).toHaveBeenCalledWith("phi4-mini");
      });
    });
  });
