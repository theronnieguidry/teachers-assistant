...
describe("Generation API Service", () => {
  ...
  describe("generateTeacherPack", () => {
    ...
    it("sends flattened inspiration without designPackContext", async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        headers: {
          get: () => "application/json",
      ...
      expect(body.designPackContext).toBeUndefined();
      expect(body.inspiration).toEqual([]);
    });
  });
  ...
  describe("streaming timeout constants", () => {
    it("should have reasonable timeout values", () => {
      ...
      expect(TIMEOUTS.STREAMING_IDLE).toBe(360000); // 6 minutes
