...
export function OllamaSetup({ open, onOpenChange }: OllamaSetupProps) {
  ...
  const renderContent = () => {
    ...
    if (step === "no-models") {
      const inTauri = isTauriContext();
      const [primaryModel, ...otherModels] = recommendedModels;
      const primaryModelName = primaryModel?.[0] || DEFAULT_RECOMMENDED_OLLAMA_MODEL;
      return (
        <div className="space-y-4">
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              {inTauri ? (
                <>
                  <Button
                    onClick={() => handlePullModel(primaryModelName)}
                    disabled={loading}
                    className="w-full"
                  >
                    {loading
                      ? "Downloading..."
                      : `Download Recommended Model (${primaryModelName})`}
                  </Button>
                  <p className="text-xs text-muted-foreground text-center">
                    Recommended for most family PCs with 8-16GB of RAM
                  </p>
                  ...
                  <div className="space-y-2">
                    {otherModels.map(([name, size, description]) => (
                      <div
                        key={name}
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Open a terminal and run: <code className="bg-muted px-1 rounded">ollama pull {DEFAULT_RECOMMENDED_OLLAMA_MODEL}</code>
                  </p>
                  <Button onClick={checkStatus} variant="outline" className="w-full mt-2">
                    Check Again
                  </Button>
