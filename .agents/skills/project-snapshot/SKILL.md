...
**CRITICAL**: The output document MUST be written to the user's Codex plans directory:
```
...
5. **Configuration**:
   ...
   - `AGENTS.md` - Get environment variables and project info
...
- `project_versions` rows with `ai_provider=Codex`: Preview supported
- Incoming requests specifying legacy provider: Remapped to OpenAI
- New writes: Never contain legacy provider values

### Migration Notes
- Codex/Anthropic removed from runtime in favor of OpenAI
- Existing Codex-generated content remains viewable
...
- [ ] Contains NO Codex/Anthropic runtime references (only in Legacy section)
