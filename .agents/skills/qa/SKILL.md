...
- **Ollama**: Free (local)
- **Codex**: Paid (Anthropic API) - ~$0.02-0.10 per full generation
...
2. **Verify API keys configured**:
   ```bash
   cd generation-api && node -e "require('dotenv').config(); console.log('Codex:', !!process.env.ANTHROPIC_API_KEY); console.log('OpenAI:', !!process.env.OPENAI_API_KEY);"
   ```
...
For **each provider** (Ollama, Codex, OpenAI):
...
- [ ] **Image sources**:
  ...
  - Codex: Uses Pixabay stock images (expected - no native image gen)
...
| Codex | Xs | Good/Fair/Poor | None/[list] |
